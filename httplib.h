...
void lprint(const char* fmt, ...)
{
	...
	va_end(args);
#ifdef _WIN32
	sprintf(buff, "%04d-%02d-%02d %02d:%02d:%02d %s err:%d\n", cur->tm_year + 1900, cur->tm_mon + 1, cur->tm_mday, cur->tm_hour, cur->tm_min, cur->tm_sec, \
		tmp, GetLastError());
#else
	sprintf(buff, "%04d-%02d-%02d %02d:%02d:%02d %s err:%d,description:%s\n", cur->tm_year + 1900, cur->tm_mon + 1, cur->tm_mday, cur->tm_hour, cur->tm_min, cur->tm_sec, \
...
// HTTP server implementation
	...
	inline socket_t Server::create_server_socket(const char *host, int port,
		...
		return detail::create_socket(
			...
			if (::bind(sock, ai.ai_addr, static_cast<socklen_t>(ai.ai_addrlen))) {
				lprint("create_server_socket bind api failed");
				return false;
			}
	...
	inline bool Client::write_request(Stream &strm, const Request &req,
		...
		if (req.body.empty()) {
			...
			else if (!req.filesEx.empty())
			{
				...
				size_t length = 0;
				std::string end = "\r\n";
				for (const auto &item : req.filesEx) {
					boundary = item.boundary;
					//获取长度
					std::string body;
					body += "--" + boundary + "\r\n";
					body += "Content-Disposition: form-data; name=\"" + item.name + "\"";
					if (item.content_provider) {
						...
						//获取文件长度
						struct stat st;
						if (::stat(item.content.c_str(), &st) != 0)
						{
							printf("file is not exist!\n");
							return false;
						}
						length += body.length() + st.st_size + end.length();
					}
					else
					{
						body += "\r\n\r\n";
				...
				auto len = std::to_string(length + end_boundary.length());
				headers.emplace("Content-Length", len);
			}
			else {
			...
			else if (!req.filesEx.empty())
			{
				...
				for (const auto &item : req.filesEx) {
					...
					if (item.content_provider) {
						...
						while (offset < end_offset) {
							if (item.content_provider(item.content, data_sink) < 0)
							{
